...
class PincherController(Node):
    def __init__(self):
        ...
        # Signo para alinear con el modelo URDF
        self.joint_sign = {
            1:  1,
            2: -1,
            3: -1,
        ...
        # Parámetros geométricos (en metros) tomados de px100_description
        # L1=44.5mm, L2=101mm, L3=101mm, L4=109mm, Lm=31.5mm, Lr=sqrt(L2^2+Lm^2)
        self.L1 = 0.0445
        self.L2 = 0.101
        self.L3 = 0.101
        self.L4 = 0.109
        self.Lm = 0.0315
        self.Lr = math.sqrt(self.L2**2 + self.Lm**2)

        # Posición TCP (x, y, z) en metros
        ...
        # Cálculo inicial de la posición TCP
    ...
    def update_tcp_position(self):
        ...
        T2 = self.dh_transform(self.Lr, 0.0, 0.0, q2)
        T3 = self.dh_transform(self.L3, 0.0, 0.0, q3)
        T4 = self.dh_transform(self.L4, 0.0, 0.0, q4)
    ...
    def initialize_motors(self, goal_positions, moving_speed, torque_limit):
        ...
        # Recalcular TCP después de inicializar
        self.update_tcp_position()
...
# ============================================================
#  INTERFAZ GRÁFICA CON PESTAÑAS (INCLUYENDO RViz + TCP)
# ============================================================
...
class PincherGUI:
    def __init__(self, controller):
        ...
        self.notebook.add(self.tab4, text='TCP / Cinemática Directa')
...
• Feedback visual de los movimientos
    ...
    # --------------------------------------------------------
    #  Pestaña 4 - TCP / Cinemática Directa
    # --------------------------------------------------------

    def setup_tab4(self):
        """Configura la pestaña 4: Posición del TCP usando cinemática directa"""
        title_label = tk.Label(self.tab4, text="Cinemática Directa - Posición del TCP",
                               font=("Arial", 14, "bold"))
        title_label.pack(pady=10)
        ...
        info_text = (
            "Esta pestaña muestra la posición del TCP (end-effector) calculada con la\n"
            "cinemática directa DH del PincherX 100 (px100_description).\n\n"
            "• Sistema de referencia: base del robot (frame 0)\n"
            "• TCP aproximado en el frame 4 de la tabla DH\n"
            "• Unidades mostradas: milímetros [mm]"
        )
        info_label = tk.Label(info_frame, text=info_text, justify=tk.LEFT,
                              font=("Arial", 10), bg="#f0f0f0", relief="solid", padx=10, pady=10)
        info_label.pack(fill='x')

        # Frame para mostrar X, Y, Z
        values_frame = tk.Frame(self.tab4)
        values_frame.pack(fill='x', padx=20, pady=20)
        ...
        for coord in ["X", "Y", "Z"]:
            ...
            value_lbl = tk.Label(row, text="0.0",
                                 font=("Arial", 12), width=12, anchor='w')
    ...
    def update_tcp_labels(self):
        ...
        # Programar siguiente actualización
        self.window.after(100, self.update_tcp_labels)

    # --------------------------------------------------------
    #  Callbacks de sliders / botones
    # --------------------------------------------------------
...
def main(args=None):
    ...
    # Si falló la inicialización (por ejemplo, puerto no abre), salir
    if not isinstance(controller, PincherController):
        return
